...
%     end
    ...
    % if file_name(1:strfind(file_name,'/')-1) == "Pseudo_periodic_synthetic"
    %     size_dataset = data.N;
    ...
    %     matlabHTM_timing_dataset = diff([time_per_dataset datetime]);
    %     fprintf ('\nProcessing Time is: %s\n',matlabHTM_timing_dataset);
    %     save (sprintf('Output/time_HTM_%s.mat',fileNames{1}(strfind(fileNames{1},'/')+1:strfind(fileNames{1},'.')-1)),'matlabHTM_timing_dataset','-append');
    % end
    %     
end
% 
